...
user can type **`@OpenRice tour`** (or any of the trigger phrases) and
...
Print the user's current state in one line (mode, installed, default
agent) before announcing Phase 1.
...
listening. Run all four "sight" states in sequence so the user sees the
sprite set:
...
> archive it; type `@OpenRice tour` to walk through this again.
>
...
the desktop API is listening. Do **not** declare OpenRice unhealthy
